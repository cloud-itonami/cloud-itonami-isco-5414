(ns security.actor
  "SecurityGuardActor — the ISCO-08 5414 community security actor as a
  `langgraph.graph/state-graph` (ADR-2607011000 / CLAUDE.md Actors
  section). One graph run = one security operation request
  ...
  The unconditional invariant: the SecurityGuardAdvisor can never
  directly commit a record the SecurityGuardGovernor refuses —
  every commit-record! call is gated behind `:decide`."
  (:require [langgraph.graph :as g]
            [langgraph.checkpoint :as cp]
            [security.advisor :as advisor]
            [security.governor :as governor]
            [security.store :as store]))

(defn build-graph
  "Build a compiled SecurityGuardActor graph. `store` implements
  `security.store/Store`. `advisor` implements
  `security.advisor/Advisor` (defaults to `mock-advisor`).
